...
that facility, $\unc{\tau}$, which correspond to
the possible   variation, or engineering tolerance,
in any physics parameter,
$\tau$,
...
\begin{align}
  \delta\vec{\Sigma}(t) = \unc{{\cal P}\left( \vec{m}_{in}\right)} = {\cal K}\left( \vec{m}_{in}\right)  
\end{align}
...
\begin{align}
  \left(\unc{\vec{m}_{out}}\right)_{\cal P^\prime} ( = \unc{{\cal P^\prime}\left( \vec{m}_{in}\right)} )= {\cal K}_1^{\prime}(\vec{m}_{in}) \label{eq:NNModError}
\end{align}
...
Eq.\eqref{eq:NNModError}. The operator to estimate
the propagation of the initial uncertainty,
${\cal K}_2^{\prime}$ requires more investigation,
